...
from utils.metrics import get_accuracy, print_arch_summary
...
from utils.models import get_target_nets

torch.set_printoptions(precision=3, linewidth=180)
wandb.login()


def init_run_logging(config, arch_configs, cli_args, cells_to_watch=[], logs_dir="logs"):
    ### init wandb
    wandb_run = None
    if config["wandb_logging"] is True:
        ...
        wandb_run = wandb.init(
            ...
            config={"config": config, "arch": arch_configs},
            group=",".join([b["name"] for b in [*config["data"]["benchmark_specs_seen_before"], *config["data"]["benchmark_specs_seen_now"]]]),
            tags=[], notes=wandb_notes
        )
        for c in cells_to_watch:
            wandb.watch(c, log="all", log_freq=100)
    
    ### save config and arch_configs locally
    ...
    with open(os.path.join(path_to_run_dir, f"full_config.json"), "w") as f:
        json.dump({"config": config, "arch_configs": arch_configs}, f, default=str, indent=4)
